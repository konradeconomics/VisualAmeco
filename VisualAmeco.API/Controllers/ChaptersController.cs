...
public class ChaptersController : ControllerBase
{
    ...
    public ChaptersController(ILookupService lookupService, ILogger<CountriesController> logger)
    {
    ...
    public async Task<ActionResult<IEnumerable<ChapterDto>>> GetAllChapters()
    {
        try
        {
            ...
            _logger.LogInformation("Returning {Count} countries.", chapters.Count());
            return Ok(chapters);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error occurred while getting all chapters.");
            return StatusCode(StatusCodes.Status500InternalServerError, "An internal error occurred while retrieving chapters.");
        }
    }
}